...
This is a module to put code shared by the other probability modules.
...
This is a place to hold collections for data.

::

    DataTuple = namedtuple('DataTuple', 'probability exponent range elapsed'.sp
    lit())
...
::
    ...
    def get_probability(trials, desired_heads, queue=None):
        ...
        if queue is not None:
            queue.put(sum(INCREMENT for count in flips if count==desired_heads)
    /float(trials))
            return
        return sum(INCREMENT for count in flips if count==desired_heads)/float(
    trials)
...
Code to help with figuring out which code needs optimizing.
